// === Draggable YouTube Button Functionality ===
...
const draggableBtn = document.getElementById('draggable-youtube-btn');

draggableBtn.addEventListener('mousedown', dragStart);
draggableBtn.addEventListener('touchstart', dragStart);
...
function dragStart(e) {
  ...
  if (e.target === draggableBtn || draggableBtn.contains(e.target)) {
    isDragging = true;
  }
}
...
function drag(e) {
  if (isDragging) {
    ...
    draggableBtn.style.transform = `translate3d(${currentX}px, ${currentY}px, 0)`;
  }
}
